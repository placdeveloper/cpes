...
/**
 * A Classe FichaCadastralNovaDAOImpl.
 */
public class FichaCadastralNovaDAOImpl extends BancoobDao implements FichaCadastralNovaDAO {
	...
	private static final String FILTRO_DATA_VIGENTE = "dataVigente";
	
	/** A constante FILTRO_DATA_VIGENTE_DATAS_IGUAIS. */
	private static final String FILTRO_DATA_VIGENTE_DATAS_IGUAIS = "dataVigenteDatasIguais";
	
	/** A constante FILTRO_PERIODO_HISTORICO. */
	private static final String FILTRO_PERIODO_HISTORICO = "periodoHistorico";
	
	/** A constante FILTRO_PERIODO_HISTORICO_DATAS_IGUAIS. */
	private static final String FILTRO_PERIODO_HISTORICO_DATAS_IGUAIS = "periodoHistoricoDatasIguais";
	
	/**
	 * Instancia um novo FichaCadastralNovaDAOImpl.
	 */
	public FichaCadastralNovaDAOImpl() {
		super(
	...
	private void ativarfiltrosConsulta(ConsultaDtoCUC<?> criterios) {
		if(isConsultaPeriodo(criterios)) {
			ativarFiltrosPeriodo(criterios);
		}
	...
	/**
	 * Verifica se as datas são iguais, no caso de consulta por PERIODO.
	 * @param criterios
	 * @return
	 */
	private boolean isPeriodoDatasIguais(ConsultaDto<?> criterios) {
		ConsultaDtoCUC<?> cri = (ConsultaDtoCUC<?>) criterios;
		return cri.getTipoConsulta() != null 	&& cri.getTipoConsulta() == TipoConsultaEnum.PERIODO && cri.getData().compareTo(cri.getDataFim()) == 0;
	}

	/**
	 * O método Ativar filtro periodo.
	 *
	 * @param criterios o valor de criterios
	 * @param classe 
	 */
	private void ativarFiltrosPeriodo(ConsultaDto<?> criterios) {
		Map<String, Object> parametros = null;
		if(criterios instanceof ConsultaDtoCUC<?>){
			parametros = new HashMap<String, Object>();
			parametros.put("data", ((ConsultaDtoCUC<?>) criterios).getData());
			parametros.put("dataFim", ((ConsultaDtoCUC<?>) criterios).getDataFim());
		}
		if(isPeriodoDatasIguais(criterios)) {
			GerenciadorFiltro.ativarFiltro(super.getEntityManager(), FILTRO_PERIODO_HISTORICO_DATAS_IGUAIS, parametros);
			GerenciadorFiltro.ativarFiltro(super.getEntityManager(), FILTRO_DATA_VIGENTE_DATAS_IGUAIS, parametros);
		}else {
			GerenciadorFiltro.ativarFiltro(super.getEntityManager(), FILTRO_PERIODO_HISTORICO, parametros);
	...
	private void desativarFiltros() {
		...
		GerenciadorFiltro.desativarFiltro(manager, FILTRO_DATA_VIGENTE_DATAS_IGUAIS);
		GerenciadorFiltro.desativarFiltro(manager, FILTRO_PERIODO_HISTORICO);
		GerenciadorFiltro.desativarFiltro(manager, FILTRO_PERIODO_HISTORICO_DATAS_IGUAIS);
	...
	private boolean isConsultaPeriodo(ConsultaDtoCUC<?> criterios) {		
		return criterios.getTipoConsulta() != null && criterios.getTipoConsulta() == TipoConsultaEnum.PERIODO;
	}
	 ...
	 */
	private boolean isConsultaEntidadeBase(ConsultaDtoCUC<?> criterios) { //TODO
		return criterios.getTipoConsulta() != null 	&& (criterios.getTipoConsulta() == TipoConsultaEnum.PERIODO || criterios.getTipoConsulta() == TipoConsultaEnum.TUDO);
	}
