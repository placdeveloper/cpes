	...
	public class ListarBemImovel extends ListarBemImovelView implements ITelaCrudBem {
		...
		private function configurarDtoConsulta(dto:ConsultaDto):void {
			...
			if(painelFiltro.comboTipoLocalizacao.selectedItem != null){
				filtro.tipoLocalizacaoBem = painelFiltro.comboTipoLocalizacao.selectedItem as TipoLocalizacaoBemVO;
			}
			
			if(painelFiltro.comboTipoUso.selectedItem != null){
				filtro.tipoUsoBem = painelFiltro.comboTipoUso.selectedItem as TipoUsoBemImovelVO;
			}
			
			if(painelFiltro.comboTipoBem.selectedItem != null){
				filtro.tipoBem = painelFiltro.comboTipoBem.selectedItem as TipoBemImovelVO;
			}
			...
			if(painelFiltro.componenteProcurarPessoa.registro != null) {
				...
				filtro.idPessoaCompartilhamentoCartorio = pessoaCompartilhamento.idPessoaCompartilhamento;
			}
		...
		private function validarCamposPesquisa():Boolean {
			if(painelFiltro.comboTipoLocalizacao.selectedItem == null
				...
				&& painelFiltro.campoNirf.text == ""
				&& painelFiltro.campoIncra.text == "")) {
					Alerta.show("É obrigatório o preenchimento do campo \"tipo localização\" e mais um campo dos demais.", "Erro", Alerta.ALERTA_ERRO);
					return false;
				}
			return true;
		}
