package br.com.sicoob.capes.cadastroBem.bemImovel {
	...
	import br.com.sicoob.capes.cadastroBem.bemImovel.abas.DadosAvaliacaoImovel;
	import br.com.sicoob.capes.cadastroBem.bemImovel.abas.DadosAvancadosImovel;
	...
	public class BemImovelEdicao extends BemImovelEdicaoView {
		...
		private function obterDefinicoes():void {
			...
			dto.dados.idTipoPessoa = ProcuraClientePlataformaCAPES.getPessoaSelecionada().codTipoPessoa;
		...
		private function obterVO():BemImovelVO {
			...
			if(avaliacao) {
				if(isImovelUrbano()) {
					vo = new BemImovelAvaliacaoUrbanoVO();
				} else {
					vo = new BemImovelAvaliacaoRuralVO();
				}
			} else {
				vo = new BemImovelVO();
			}
			...
			iterarAbas(function(aba:IAba):void {
				if(Container(aba) != obterAbaDadosBasicos()){
					if(Container(aba) == obterAbaDocumentos()) {
						if(ProcuraClientePlataformaCAPES.getPessoaSelecionada() != null) {
							valor = ProcuraClientePlataformaCAPES.getPessoaSelecionada().utilizaGedGft;
						} else {
							valor = false;
						}
					}
