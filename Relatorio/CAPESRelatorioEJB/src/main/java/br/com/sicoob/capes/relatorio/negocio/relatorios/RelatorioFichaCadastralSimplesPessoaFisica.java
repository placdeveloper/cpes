...
public class RelatorioFichaCadastralSimplesPessoaFisica extends RelatorioFichaCadastralSimples{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	
	public RelatorioFichaCadastralSimplesPessoaFisica(FichaCadastralVO fichaCadastralVO) throws BancoobException {
	...
	protected Object recuperaRelatorioDadosPessoais() throws JRException {
		return recuperarRelatorio("FichaCadastralSimples_DadosPessoaisPF.jasper");
	}
	...
	protected EnderecoBase recuperaEnderecoResidencialPessoa(List<EnderecoBase> enderecos)throws BancoobException {
		for (EnderecoBase enderecoBase : enderecos) {
			if(enderecoBase.getTipoEndereco().getCodigo().equals(getEnderecoResidencial())){
				return enderecoBase;
			}
		}
		return null;
	...
	protected Telefone recuperaTelefoneResidencialPessoa(List<Telefone> telefones) throws BancoobException {
		if (telefones != null && !telefones.isEmpty()) {
			...
			for (Telefone telefone : telefones) {
				if (telefone.getTipoTelefone().getCodigo().equals(getTelefoneResidencial())) {
					return telefone;
				}
			}
		}
