...
public class AlterarFonteRendaServicoEJB extends ServicosComprovaveisEJB {
	...
	protected RetornoTransacaoObjeto executarTransacao(Mensagem mensagem, Map<String, Parametro> parametros) throws BancoobException {
		...
		// Cria o DTO para alteração da renda
		...
		dto.setIdFonteRenda(idFonteRenda.longValue());
		...
		alterarFonteRenda(dto);
		encaminharFluxo(pessoa.getIdPessoa(), pessoa.getIdInstituicao(), pessoa.getIdUnidadeInst(), dto.getIdFonteRenda(), pessoa.getIdInstituicao(), idUsuario);
	...
	public void alterarFonteRenda(FonteRendaDTO fonteRenda) throws BancoobException {
		obterDelegateInclusao().alterar(fonteRenda);
	}
